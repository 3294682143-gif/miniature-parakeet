...
def reg(
    ...
    return {
        ...
        "files": files,
        "command_entry": (
            "python -m math_agent.cli" if category == "A" else "script/module entry"
        ),
...
FUNCTION_AUDIT_REGISTRY: list[dict[str, Any]] = [
    ...
    reg(
        ...
        "A",
        "integrated",
        ["src/math_agent/clients/interns1_client.py"],
    ),
    reg(
        "A07",
        "fast/full/tool-first modes",
        "A",
        "integrated",
    ...
    reg(
        ...
        "B",
        "integrated",
        ["src/math_agent/tools/sympy_tools.py"],
    ),
    reg(
        "B02",
        "Python sandbox tool",
        "B",
        "integrated",
        ["src/math_agent/tools/python_sandbox.py"],
    ),
    reg(
        "B03",
        "answer_normalizer",
        "B",
        "integrated",
        ["src/math_agent/tools/answer_normalizer.py"],
    ),
    reg("B04", "tool call record", "B", "integrated", ["src/math_agent/schemas.py"]),
    reg(
        "B05", "tool error fallback", "B", "integrated", ["src/math_agent/pipeline.py"]
    ),
    reg(
        "C01",
        "Formatter Repair",
        "C",
        "integrated",
        ["src/math_agent/harness/formatter_repair.py"],
    ),
    reg(
        "C02",
        "sanitize_boxed",
    ...
    reg(
        "C06",
        "boxed_42_fallback detection",
        "C",
        "integrated",
        ["src/math_agent/evaluation/metrics.py"],
    ),
    reg(
        "D01",
        "ProofGuardian core",
        "D",
        "integrated",
        ["src/math_agent/proof/proof_guardian.py"],
    ),
    reg(
        "D02",
        "ProofRubricScore",
        "D",
        "integrated",
        ["src/math_agent/proof/proof_rubric.py"],
    ),
    reg(
        "D03",
    ...
    reg(
        ...
        "D",
        "preview",
        ["src/math_agent/control/proof_guardian_hook.py"],
    ),
    reg(
        "D05",
    ...
    reg(
        "D06",
        "proof scoring in verifier_scoring",
        "D",
        "integrated",
        ["src/math_agent/verification/verifier_scoring.py"],
    ),
    reg(
        "D07",
        "proof demo script",
    ...
    reg(
        ...
        "E",
        "integrated",
        ["src/math_agent/harness/skill_registry.py"],
    ),
    reg(
        "E03",
        "proof/equation/calculation/etc skills",
        "E",
        "integrated",
        ["skills/proof.skill.md", "skills/equation.skill.md"],
    ),
    ...
    reg(
        "H01", "HardModePolicy", "H", "integrated", ["src/math_agent/control/policy.py"]
    ),
    reg(
        "H02",
        "hard-mode levels",
        "H",
        "integrated",
        ["src/math_agent/control/hard_mode.py"],
    ),
    reg(
        "H03",
        "candidate_budget",
        "H",
        "preview",
        ["src/math_agent/control/candidate_budget.py"],
    ),
    reg(
        "H04", "verifier_level", "H", "preview", ["src/math_agent/control/hard_mode.py"]
    ),
    reg(
        "H05", "require_trace", "H", "preview", ["src/math_agent/control/hard_mode.py"]
    ),
    reg(
        "H06",
        "proof_guardian flag",
    ...
    reg(
        "H10",
        "pipeline metadata hook",
        "H",
        "integrated",
        ["src/math_agent/control/pipeline_hook.py"],
    ),
    reg(
        "I01",
        ...
        "I",
        "standalone",
        ["scripts/run_weighted_voting_demo.py"],
    ),
    reg(
        "I02",
        "controlled weighted voting",
        "I",
        "integrated",
        ["src/math_agent/control/weighted_voting_hook.py"],
    ),
    reg(
        "I03",
        ...
        "I",
        "integrated",
        ["src/math_agent/verification/weighted_voting.py"],
    ),
    reg(
        "I04",
        "verifier scoring",
    ...
    reg(
        "I06",
        "final candidate selection trace",
        "I",
        "integrated",
        ["src/math_agent/verification/weighted_voting.py"],
    ),
    reg(
        ...
        "J",
        "integrated",
        ["src/math_agent/harness/budget_scheduler.py"],
    ),
    ...
    reg(
        "K02", "metrics.py", "K", "integrated", ["src/math_agent/evaluation/metrics.py"]
    ),
    reg(
        "K03",
        "error_taxonomy.py",
        "K",
        "integrated",
        ["src/math_agent/evaluation/error_taxonomy.py"],
    ),
    reg(
        "K04", "build_eval_report", "K", "standalone", ["scripts/build_eval_report.py"]
    ),
    reg(
        "L01",
        "run_regression_gate",
    ...
    reg(
        "L04",
        "pytest/format/type/safety gates",
        "L",
        "integrated",
        ["scripts/run_regression_gate.py"],
    ),
    reg(
        ...
        "M",
        "integrated",
        ["src/math_agent/debugger/report.py"],
    ),
    reg(
        "M02",
        "FailureCase",
        "M",
        "integrated",
        ["src/math_agent/debugger/failure_attribution.py"],
    ),
    reg(
        "M03",
        "FailureCluster",
        "M",
        "integrated",
        ["src/math_agent/debugger/root_cause.py"],
    ),
    reg(
        "M04",
        "DebuggerReport",
    ...
    reg(
        ...
        "N",
        "integrated",
        ["src/math_agent/harness/trace_reader.py"],
    ),
    ...
    reg(
        "N04",
        "build_timeline/summarize_trace",
        "N",
        "integrated",
        ["src/math_agent/harness/replay.py"],
    ),
    reg(
        "N05",
    ...
    reg(
        "N06", "trace redaction", "N", "integrated", ["src/math_agent/logging_utils.py"]
    ),
    ...
    reg(
        ...
        "O",
        "integrated",
        ["src/math_agent/harness/demo_adapter.py"],
    ),
    reg(
        "P01",
        "generate_demo_pack",
    ...
    reg(
        "P02",
        "demo evidence markdowns",
        "P",
        "integrated",
        ["src/math_agent/evidence/demo_pack.py"],
    ),
    reg(
        "Q01",
    ...
    reg(
        ...
        "Q",
        "integrated",
        ["src/math_agent/evaluation/hard_mode_ablation.py"],
    ),
    reg(
        "R01",
    ...
    reg(
        "R02",
        "dry_run report/run_id/config_snapshot",
        "R",
        "integrated",
        ["src/math_agent/submission/dry_run.py", "src/math_agent/submission/report.py"],
    ),
    reg(
        "R03",
        "no official_results guarantee",
        "R",
        "integrated",
        ["scripts/run_official_dry_run.py"],
    ),
    ...
    reg(
        ...
        "T",
        "present",
        ["evolution/manifests/change_manifest_template.yaml"],
    ),
    reg(
        "U01",
        ...
        "U",
        "integrated",
        ["scripts/check_project_safety.py"],
    ),
    reg(
        "U02",
        ".env/key/bearer/output detection",
        "U",
        "integrated",
        ["scripts/check_project_safety.py"],
    ),
    reg(
        "U03",
        "official_results/__pycache__/pytest_cache detection",
        "U",
        "integrated",
        ["scripts/check_project_safety.py"],
    ),
    ...
    reg(
        "V02",
        "current_baseline_audit",
        "V",
        "present",
        ["docs/current_baseline_audit.md"],
    ),
    reg(
        "V03",
        "git/local/CI checks",
    ...
    reg(
        "W02",
        "final_report/architecture/replay/submission docs",
        "W",
        "present",
        [
    ...
    reg(
        "W03",
        "hard_mode_control/proof_guardian/full_system_audit docs",
        "W",
        "present",
        [
            "docs/hard_mode_control.md",
            "docs/proof_guardian.md",
            "docs/full_system_audit.md",
        ],
    ),
    reg(
        "X01",
        "full_system_audit script",
        "X",
        "standalone",
    ...
    reg(
        ...
        [
            ...
            "memory/regression_cases.yaml",
            "memory/route_stats.json",
            "memory/skill_success_stats.json",
            "memory/verifier_failures.json",
            "memory/answer_cluster_stats.json",
        ],
    ),
    reg(
        "H11",
        "Candidate Budget Preview",
        "H",
        "preview",
        ["src/math_agent/control/candidate_budget.py"],
    ),
    reg(
        "H12",
        "Verifier Routing Preview",
        "H",
        "preview",
        ["src/math_agent/control/verifier_routing.py"],
    ),
    reg(
        "I07",
    ...
    reg(
        ...
        "K",
        "standalone",
        ["src/math_agent/evaluation/shadow_eval_artifacts.py"],
    ),
    reg(
        "L05", "shadow eval gate", "L", "integrated", ["scripts/run_regression_gate.py"]
    ),
    reg(
        "M06",
        "debugger evidence files",
        "M",
        "standalone",
    ...
    reg(
        "P03",
        "demo script and reports",
        "P",
        "present",
        [
            "docs/demo_script.md",
            "docs/ablation_report.md",
            "docs/hard_mode_report.md",
            "docs/proof_rubric_report.md",
            "docs/failure_debug_report.md",
            "docs/architecture_summary.md",
            "docs/risk_control_summary.md",
        ],
    ),
    reg(
        "Q03",
    ...
    reg(
        "R04",
        "official-like input + dry_run jsonl",
        "R",
        "standalone",
        ["data/preofficial_sample.jsonl"],
    ),
    reg(
        "S03",
        "README_SUBMISSION and snapshot note",
        "S",
        "present",
        ["docs/README_SUBMISSION.md", "docs/src_snapshot_note.md"],
    ),
    reg(
        "T03",
        "candidate archive docs",
        "T",
        "present",
        ["evolution/candidates/README.md", "evolution/evidence/README.md"],
    ),
    reg(
        "V04", "line count summary", "V", "integrated", ["scripts/full_system_audit.py"]
    ),
    reg(
        "W04", "docs skeleton tests", "W", "integrated", ["tests/test_docs_skeleton.py"]
    ),
    reg(
        "X03",
        "line_count_report/quality/smoke/report outputs",
        "X",
        "integrated",
        ["scripts/full_system_audit.py"],
    ),
...
def write_outputs(
    ...
    inv_md = [
        ...
        "## A. Stable Core / 主解题内核",
        "",
        "| id | name | category | status | files | risk_boundary |",
        "|---|---|---|---|---|---|",
    ]
    for x in inv:
        inv_md.append(
...
def main() -> int:
    ...
    smoke = [
        (
            run_cmd(c, root, 1200)
            if (not c[1].startswith("scripts/") or (root / c[1]).exists())
            else CheckResult(" ".join(c), c, 127, "MISSING", "script not found")
        )
        for c in smoke_cmds
    ]

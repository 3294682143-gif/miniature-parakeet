...
class MathAgentPipeline:
    ...
    def solve(self,question:str,question_id:str|None=None)->SolveResult:
        ...
        try:
            ...
            vtxt=verifier.run(question)
            passed=True if self.mock else ('pass' in vtxt.lower())
            verification=Verification(method='self_review',passed=passed,notes=vtxt)
            if self.enable_tools:
                ...
                if tv is not None and tvf is not None: final,verification,status=tv,tvf,'success'
            rounds=0; current=draft
            while not verification.passed and rounds<self.max_refine_rounds:
                rounds+=1; current=refiner.run(current); final=extract_boxed_answer(current) or final; vtxt=verifier.run(question); verification=Verification(method='self_review',passed=('pass' in vtxt.lower()),notes=vtxt)
            explanation=explainer.run(question)
            if not verification.passed and status=='success': status='partial'
            result=SolveResult(question_id=qid,domain=route_dict.get('domain','unknown'),problem_type=route_dict.get('problem_type','unknown'),problem_parse=ProblemParse(goal=question,givens=[question],symbols=[]),solution_plan=[str(plan)],visible_solution_steps=[current],tool_trace=traces,final_answer=FinalAnswer(type='text',value=final,boxed=f"\\boxed{{{final}}}" if final else ''),verification=verification,didactic_hint=explanation,confidence=max(0.0,min(1.0,route_dict.get('confidence',0.5) or 0.5)),status=status,error=None)
        except Exception as exc:

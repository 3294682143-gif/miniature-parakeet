...
def render_failure_debug_report(report: DebuggerReport) -> str:
    ...
    lines.extend([f"- {c.key}: {c.count} ({c.severity}) owner={c.suggested_owner}" for c in report.clusters])
    lines.append("## 7. Representative Failures")
    lines.extend([f"- {c.id}: {c.failure_category} status={c.status}" for c in report.representative_failures])
    lines.append("## 8. Root Cause Mapping")
    lines.extend([f"- {c.id}: {infer_root_cause(c).root_cause}" for c in report.representative_failures])
    ...
    lines.extend(["## 12. Official Submission Warning", "- This debugger output is not official scoring."])
    return "\n".join(lines) + "\n"
...
def render_demo_case_list(report: DebuggerReport) -> str:
    def _pick(categories: set[str]) -> list[str]:
        ...
        return [f"- id={c.id} domain={c.domain} difficulty={c.difficulty} failure_category={c.failure_category} suggested demo angle={infer_severity(c)}-{infer_root_cause(c).owner}" for c in items[:3]]
    ...
    lines = [
        ...
        *(_pick({"json_invalid", "missing_final", "dirty_boxed", "boxed_42_fallback", "formatter_repair_failed"}) or ["- none"]),
